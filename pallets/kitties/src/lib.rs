...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		type KittyIndex: Parameter
			+ Member
			// u32类型，https://paritytech.github.io/substrate/master/sp_arithmetic/traits/trait.AtLeast32Bit.html
	...
	pub type KittyAll<T: Config> = StorageMap<
		...
		Blake2_128Concat,
		T::AccountId,
		BoundedVec<Kitty, T::MaxKittyIdLength>,
		ValueQuery,
	>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn transfer(
			...
			// 存储转移后的kitty前，先将转移前的kitty从存储中删除
			KittyAll::<T>::try_mutate(&who, |kitty_owned| {
				if let Some(index) = kitty_owned.iter().position(|kitty| kitty == &exist_kitty) {
					kitty_owned.swap_remove(index);
					return Ok(())
				}
				Err(())
			})
