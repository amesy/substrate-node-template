...
fn breed_kitty_failed_token_not_enough_to_reversed() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(KittiesModule::breed(Origin::signed(3), 0, 1), Error::<Test>::TokenNotEnough);
	})
}
...
fn transfer_kitty_works() {
	new_test_ext().execute_with(|| {
		...
		// 预期账户1下没有kitty，账户2下有2个kitty
		assert_eq!(KittyAll::<Test>::take(1).len(), 0);
		assert_eq!(KittyAll::<Test>::take(2).len(), 2);
	})
}
...
fn transfer_kitty_failed_not_owner() {
	new_test_ext().execute_with(|| {
		...
		// 账户1把账户2下的编号为1的kitty转移给自己，由于账户1不是kitty编号为1的kitty的owner，预期将报错
		assert_noop!(KittiesModule::transfer(Origin::signed(1), 1, 1), Error::<Test>::NotOwner);
	})
}
...
fn transfer_kitty_failed_token_not_enough_to_reversed() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(KittiesModule::transfer(Origin::signed(1), 0, 3), Error::<Test>::TokenNotEnough);
	})
}
